...
function freezingDay(
  ...
  const available = calculateFreezingAvailability(previousDays, date).find(
    (position) =>
      position.originDayId === source.id && position.productId === product.productId,
  )
...
describe('Packing to Freezing lifecycle', () => {
  it('treats historical records without process as Packing', () => {
    const historical = JSON.parse(
      JSON.stringify(
        packingDay('2026-09-07', 100),
        (key, value) => (key === 'process' ? undefined : value),
      ),
    ) as ProductionDay
  ...
  it('reduces the pending balance without losing its Packing origin', () => {
    ...
    expect(freezing.productionDay.receivedBalanceLots[0]?.originDayId).toBe(packing.id)
  ...
  it('carries the same traceable balance across days and weeks', () => {
    ...
    expect(monday.productionDay.receivedBalanceLots[0]?.originDayId).toBe(saturday.id)
  ...
  it('keeps the Packing balance ledger separate from Freezing uses', () => {
    ...
    expect(calculateOutstandingBalances([packing, freezing.productionDay]))
