...
import { isFreezingProductionDay, isPackingProductionDay } from './productionProcess'
import type {
  Kg100,
  ProductionDay,
  SummaryGroupId,
...
export function calculateFreezingAvailability(
  ...
  const sourceDays = productionDays.filter(
    (day) =>
      isClosedBalancedPackingDay(day) &&
      (!asOfDate || day.date <= asOfDate),
  )
  ...
  return sourceDays.flatMap((originDay) =>
    originDay.lines
      .filter((line) => line.declaredFinishedKg100 > 0)
      .map((line) => {
        ...
        const differenceKg100 = kg100(
          line.declaredFinishedKg100 - processedTotalKg100,
        )
        ...
        return {
          ...
          generatedKg100: line.declaredFinishedKg100,
          processedDayKg100,
          processedNightKg100,
          processedTotalKg100,
...
function aggregateComparisonRows(
  ...
  for (const position of positions) {
    ...
    const label = dimension === 'family' ? position.familyName : position.productName
...
export function calculateFreezingComparison(
  ...
  const unlinkedByFamily = new Map<
    string,
    { label: string; kg100: Kg100 }
  >()
  const unlinkedByProduct = new Map<
    string,
    { label: string; kg100: Kg100 }
  >()
    ...
    productionDays
      ...
      .flatMap((day) =>
        day.lines.map((line) => {
          ...
          const unlinkedKg100 = kg100(
            Math.max(physicalKg100 - linkedKg100, 0),
          )
          unlinkedByFamily.set(
            line.familyId,
            {
              label: line.familyName,
              kg100: kg100(
                (unlinkedByFamily.get(line.familyId)?.kg100 ?? 0) +
                  unlinkedKg100,
              ),
            },
          )
          unlinkedByProduct.set(
            line.productId,
            {
